...
#######################################################################	
#	
#	Reason about any phenomena I see:
#		
#	
#	
#
...
def main():
	...
	# plt.figure(1)
	ax.scatter(labeledMatrix[:,0], labeledMatrix[:,1], labeledMatrix[:,2], c='b', marker='+')
	# format plot
	# marginRatio = 0.05
	# axX_min = round( zipcodeMin - zipcodeRange*marginRatio )
	# axX_max = round( zipcodeMax + zipcodeRange*marginRatio )
	# ax[2].set_xbound( axX_min, axX_max )
	# axY_min = round( num_incidentsMin - num_incidentsRange*marginRatio )
	# axY_max = round( num_incidentsMax + num_incidentsRange*marginRatio )
	# print str(axY_min)+":"+str(axY_max)
	# ax[2].set_ybound( axY_min, axY_max )
	# ax[2].set_ylim( 1e-1, 1e6 )
	# ax[2].set_yscale('log')
	# ax[2].grid(True)
	# ax[2].set_xlabel( 'Zipcode' )
	# ax[2].set_ylabel( '')
	# ax[2].set_title('As a function of zipcode')
	plt.show()
