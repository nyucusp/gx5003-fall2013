...
create_zip = "create table zips (zip_code int not null, area float not null, pop_by_zip int)"
create_bor = "create table bor (zip_code int not null, name text not null)"
create_inc = "create table inc (address varchar(500), zip_code int not null, num_inc int)"

#Stop the warnings from appearing from the drop if exists commmand
#import warnings
#warnings.filterwarnings("ignore", "Unknown table.*")
#So I can create the table over again, use the drop if exists command (for the purpose of testing)
cur.execute(create_zip)
cur.execute(create_bor)
cur.execute(create_inc)
...
for line in zip_data_read: 
    ...
    if zip_val_list[3] != 'H' and pop_by_zip != ' ' and pop_by_zip != '': #ignores dirty data with HH zipcode or emptly pop value
        insert_command = "insert into zips values(" + "'" + zip_val + "'" + "," + "'" + area + "'" + "," + "'" + pop_by_zip + "'"  ");"
