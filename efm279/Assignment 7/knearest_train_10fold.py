...
def load_base(fn):
    a = np.loadtxt(fn, np.float32, delimiter=',', converters={ 0 : lambda ch : ord(ch)-ord('A') }) #Convert letters to digits
    samples, responses = a[:,1:], a[:,0]
    return samples, responses

class LetterStatModel(object):
    class_n = 26
    train_ratio = 0.8 # First 16000 samples are used for training
...
if __name__ == '__main__':
    ...
    kf = KFold(train_n, n_folds=10, indices=True)
    summary=[]
    summary.append(['fold #','percent accuracy','elapsed time'])
    carmelo_anthony_sucks=1
    for test,train in kf:
        ...
        if '--load' in args:
            ...
            model.train(samples[train[0]:train[len(train)-1]], responses[train[0]:train[len(train)-1]])
        n4=dt.datetime.now()
        print 'elapsed training time is ' +str(((n4-n3).seconds)) +' seconds'
        
        print 'testing...'
        train_rate = np.mean(model.predict(samples[train[0]:train[len(train)-1]]) == responses[train[0]:train[len(train)-1]])
        test_rate  = np.mean(model.predict(samples[train[0]:train[len(train)-1]]) == responses[train[0]:train[len(train)-1]])
        ...
        summary.append([str(carmelo_anthony_sucks),str(train_rate),str(((n4-n3).microseconds))])
        carmelo_anthony_sucks+=1

    with open('KNearest10fold.csv', 'wb') as fp:
