...
createCommand = "CREATE Incidents_table (incident_address varchar(255), incident_zip(5) int not null, key int not null, primary key(incident_address)"
cur.execute(createCommand)

open('incidents.csv')
incidents_csv = csv.reader(file('incidents.csv', 'rb'))
incidents_csv.readline()
for row in zipcodes_csv:
    if row[1] != ' ':#select only if associated with incident zipcode
        ...
        key = row[2]
        population_perZip = row[10]
        insertCommand = "INSERT into Incidents_table values(" + incident_address + "," + "'" + str(incident_zip) + "'" + "," + str(key) + ");"
        cur.execute(insertCommand)
