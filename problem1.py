...
with db:
    ...
    createCommand = "CREATE table Incidents_table (incident_address varchar(50), incident_zip int not null, key int not null, primary key(address))"
    cur.execute(createCommand)
    ...
    for row in zipcodes_csv:
        if row[1] != ' ':#select only if associated with incident zipcode
            ...
            key = row[2]
            population_perZip = row[10]
            insertCommand = "INSERT into Incidents_table values(" + address + "," + "'" + str(izip) + "'" + "," + str(key) + ");"
            cur.execute(insertCommand
